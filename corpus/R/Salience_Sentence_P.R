...
wdth <- 1024-50  # width of the png   | # MV: 50 pix offset 
...
wrds <- c('She is the author of a series of successful high school biology textbooks.',
          ...
          'He never really understood how anyone could survive without literature.',
...
for (i in (1:length(wrds))) {
  ...
  bmp(filename = paste(flnm[i], crtv, ".bmp", sep=""), width = wdth, height = hght)
    par(mar=c(0,0,0,0))
